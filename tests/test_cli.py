...
def dummy_output_file():
    ...
    before.
    """
    temp_file = tempfile.NamedTemporaryFile("r", suffix=".pdb", delete=False)
...
def test_simple(input_file, output_file):
    ...
    """
    run_cli(["-v", "-i", input_file, "-o", output_file, "-c", str(PH)])
...
def test_pdbfile_missing_bond_order():
    ...
    input_file = tempfile.NamedTemporaryFile("w", suffix=".pdb", delete=False)
    strucio.save_structure(input_file.name, model)
    input_file.close()

    output_file = tempfile.NamedTemporaryFile("r", suffix=".pdb", delete=False)
    output_file.close()

    with pytest.warns(UserWarning, match="For some bonds the bond order is unknown"):
        run_cli(["-v", "-i", input_file.name, "-o", output_file.name, "-c", str(PH)])
...
def test_small_molecule(format):
    ...
    """
    sd_file = mol.SDFile.read(join(data_dir(), "TYR.sdf"))
    ref_model = mol.get_structure(sd_file)
    model = ref_model[ref_model.element != "H"]

    input_file = tempfile.NamedTemporaryFile("w", suffix=f".{format}", delete=False)
    strucio.save_structure(input_file.name, model)
    input_file.close()

    output_file = tempfile.NamedTemporaryFile("r", suffix=f".{format}", delete=False)
    output_file.close()
...
def test_std_in_out(input_file, output_file):
    ...
    completed_process = subprocess.run(
        ["hydride", "-v", "-I", in_format, "-O", out_format, "-c", str(PH)],
...
def test_extra_fragments(output_file, res_name):
    ...
    frag_temp_file = tempfile.NamedTemporaryFile("w", suffix=".bcif", delete=False)
    ...
    input_temp_file = tempfile.NamedTemporaryFile("w", suffix=".bcif", delete=False)
...
def test_ignored_residues(input_file, output_file):
    ...
    run_cli(
        ...
        + list(
            itertools.chain(*[("-g", "A", str(res_id)) for res_id in ignored_residues])
        )
    )
    ...
    for res_id in ignored_residues:
        assert not ((test_model.element == "H") & (test_model.res_id == res_id)).any()
...
def test_pbc(input_file, output_file):
    ...
    """
    run_cli(["-v", "-i", input_file, "-o", output_file, "-c", str(PH), "--pbc"])
...
def test_invalid_iteratons(input_file, dummy_output_file):
    with pytest.raises(SystemExit) as wrapped_exception:
        run_cli(["-i", input_file, "-o", dummy_output_file, "-n", str(-1)])
...
def test_invalid_file_data(input_file, dummy_output_file):
    ...
    temp_file = tempfile.NamedTemporaryFile("wb", suffix=in_format, delete=False)
...
def test_missing_ignore_residue(input_file, dummy_output_file):
    with pytest.raises(SystemExit) as wrapped_exception:
        run_cli(["-i", input_file, "-o", dummy_output_file, "-g", "A", "42"])
    assert wrapped_exception.value.code == 1
